...
SWEP.PrintName			= "Spore Grenade"	
SWEP.ClassName			= "grenade_spores"			
...
function SWEP:Initialize()
    ...
    killicon.Add( "weapon_spores", "", color_white ) --aimbot/killico
    end
...
function SWEP:CreateGrenade()
	...
	if IsValid(self.Owner) && IsValid(self.Weapon) then
		...
		if (SERVER) then
			...
			local ent = ents.Create("ent_fungus")
...
function SWEP:Think()
	...
	if SERVER then
		if self.Owner:GetAmmoCount( "slam" ) <= 0 then
			self.Owner:StripWeapon( "grenade_spores" ) 
		end
	end
end
...
function SWEP:DrawWeaponSelection(x,y,wide,tall,alpha)
    draw.SimpleText("Spore Grenade","Arial",x + wide/2,y + tall*0.35,team.GetColor( LocalPlayer():Team() ),TEXT_ALIGN_CENTER) -- is this right?
end