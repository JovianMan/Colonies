...
function ENT:Think()
	...
	if CLIENT then 
		...
		function EFFECT:Init(ed)
			...
			for i=1,60 do
				...
				if (smoke) then
					...
					smoke:SetVelocity(VectorRand():GetNormal()*math.random(100, 300)/3)
				...
				if (smoke) then
					...
					smoke:SetVelocity(VectorRand():GetNormal()*math.random(100, 300)/5)
					smoke:SetRoll(math.Rand(0, 360))
					smoke:SetRollDelta(math.Rand(-2, 2))
					smoke:SetDieTime(6)
					smoke:SetLifeTime(0)
			...
			for i=1,10 do
				...
				if (sparks) then
					...
					sparks:SetVelocity(VectorRand():GetNormal()*math.random(300, 500)/100)
		...
		effects.Register( EFFECT, "rocket_explode", true )
		...
		function EFFECT:Init(ed)
			...
			for i=1,10 do
				...
				if (smoke) then
					...
					smoke:SetVelocity(VectorRand():GetNormal()*math.random(100, 300)/100*i)
					smoke:SetRoll(math.Rand(0, 360))
					smoke:SetRollDelta(math.Rand(-2, 2))
					smoke:SetDieTime(1)
					smoke:SetLifeTime(0)
		...
		effects.Register( EFFECT, "rocket_smoke", true )
	end
end
