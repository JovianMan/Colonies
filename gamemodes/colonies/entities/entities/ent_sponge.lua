...
function ENT:Initialize()
	if SERVER then
		...
		self:SetColor(c)
		self:SetModelScale(self:GetModelScale()*0.3,0)
...
function ENT:findTarget()
	...
	for i,v in pairs( ents.FindByClass( "npc_*" ) ) do
		if v ~= self.Owner and IsFriendEntityName( v:GetClass() ) == false and v.sponge == nil then
			local dist = pos:Distance( v:GetPos() )
			if dist <= d then d = dist ent = v end
		end
	end
	for n,f in pairs( player.GetAll() ) do
		if f:Team() ~= self.Team and f.sponge == nil then
...
function ENT:Explode(dmg,rng)
	...
	if IsValid(self:GetParent()) then
		if IsValid(self:GetParent().sponge) then
			self:GetParent().sponge = nil
		end
	end
end

function ENT:Touch(ent)
	if ent:IsPlayer() and ent:Team() ~= self.Owner:Team() and self.Stuck == false and ent.sponge == nil then
